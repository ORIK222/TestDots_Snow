...
public class Reader : IBufferReader {
  ...
  public long GetContentLength() {
    ...
    public class BufferedStreamLoader : IUriLoader, IDisposable
    {
        private Stream glbBase;
        private string uriBase;
        private int bufferSize;
        private Stream bufferedStream;

        /// glbPath is the .gltf or .glb file being read; or null.
        public BufferedStreamLoader(Stream glbBase, string uriBase, int bufferSize = 4096)
        {
            this.glbBase = glbBase;
        ...
        public IBufferReader Load(string uri)
        {
            ...
            if (uri == null)
            {
                var range = GlbParser.GetBinChunk(glbBase, GlbParser.GetFileEncoding(glbBase));
                bufferedStream = new SubStream(glbBase, range.start, range.length);
            }
            else
            {
                bufferedStream = File.OpenRead(Path.Combine(uriBase, uri));
            }

            return new BufferedStreamReader(bufferedStream, bufferSize, bufferedStream.Length);
        }
        ...
        public void Dispose()
        {
            if (this.glbBase != null)
            {
                this.glbBase.Dispose();
                this.glbBase = null;
            }
